...
#if defined(NDEBUG)
# error "Calm cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Calm: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Calm Signed Message:\n";
...
{
    // Calm: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Calm: 1 day
static const int64 nTargetSpacing = 1 * 45; // Calm: 45 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Calm: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Calm: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Calm: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CalmMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CalmMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CalmMiner(CWallet *pwallet)
{
    printf("CalmMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CalmMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CalmMiner, pwallet));
}
